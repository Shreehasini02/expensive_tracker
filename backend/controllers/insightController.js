...
const buildMonthlyInsight = async (userId) => {
  const data = await pool.query(
    ...
    SELECT
      ...
      COALESCE(
        (SELECT json_agg(breakdown) FROM breakdown),
        '[]'
      ) AS breakdown,

      COALESCE(
        (SELECT json_agg(trend) FROM trend),
        '[]'
      ) AS trend
    `,
    [userId]
...
const buildSavingsTips = async (userId) => {
  ...
  const content = await generateSavingsTips({
    topCategories: top.rows.map((r) => ({
      ...
      transactionCount: parseInt(r.count || 0, 10),
    })),
...
const buildBudgetAlert = async (userId, categoryId) => {
  if (!categoryId) {
    const err = new Error(
      "categoryId is required for budget alerts"
    );
  ...
  const content = await generateBudgetAlert({
    ...
    budgetAmount: parseFloat(b.amount || 0),
